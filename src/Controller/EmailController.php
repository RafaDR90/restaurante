...
class EmailController extends AbstractController
{
    ...
    public function sendEmailDepartamento($htmlContent): Response
    {
        $emailUser = $this->getUser()->getEmail();
        $transport = Transport::fromDsn($_ENV['MAILER_DSN']);
        $mailer = new Mailer($transport);
        $email = (new Email())
            ...
            //->replyTo('fabien@example.com')
            //->priority(Email::PRIORITY_HIGH)
            ->subject('Pedido realizado con éxito por '.$emailUser)
            ->text('Sending emails is fun again!')
            ->html($htmlContent);
    ...
    public function crearHTMLDepartamento($productos)
    {
        $emailUsuario = $this->getUser()->getUserIdentifier();
        $html = '<!DOCTYPE html>
            <html lang="en">
            <head>
                <meta charset="UTF-8">
            ...
            <body>
                <h1>Productos comprados por '.$emailUsuario.'</h1>
                <div class="container">';
        $totalProductos = 0;
        foreach ($productos as $producto) {
            // Muestra producto nombre, producto cantidad
